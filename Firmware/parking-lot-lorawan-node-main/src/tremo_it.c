...
{
    if (gpio_get_interrupt_status(GPIO_USING, ir_pin) == SET) {
        gpio_clear_interrupt(GPIO_USING, ir_pin);
        interrupt_flag = 1;
        count=0;
    }

    if (gpio_get_interrupt_status(GPIO_USING, ir_pin_slot4)==SET){

        gpio_clear_interrupt(GPIO_USING, ir_pin_slot4);
        interrupt_flag=1;
        count=0;
